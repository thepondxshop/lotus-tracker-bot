...
def normalize_domain(v):
    v = re.sub(r"^https?://", "", str(v or "").strip(), flags=re.I)
    return v.strip("/")
...
def classify_game(title):
    t = clean(title).lower()
    if not t or any(x in t for x in UNSUPPORTED): return None
    if "one piece card game" in t or "one piece tcg" in t or ONE_PIECE_CODE.search(title or ""):
        return "One Piece"
    for game, terms in GAME_TERMS.items():
        if any(x in t for x in terms): return game
    return None

def category(title):
    t = clean(title).lower()
    if ONE_PIECE_CODE.search(title or ""): return "SINGLE"
    if POKEMON_NUMBER.search(title or "") and ("pokemon" in t or "pokémon" in t): return "SINGLE"
    if any(x in t for x in SINGLE): return "SINGLE"
    if any(x in t for x in SEALED): return "SEALED"
    if any(x in t for x in ACCESSORY): return "ACCESSORY"
    return "UNKNOWN"

def product_type(title):
    c = category(title)
    if c == "SINGLE": return "Single Card"
    t = clean(title).lower()
    mapping = (
...
def language(f):
    return {"GLOBAL_STANDARD":"English","JP":"Japanese","KR":"Korean","CN":"Simplified Chinese","UNKNOWN":"Unknown"}.get(f,"Unknown")

def jsonld_objects(text):
    out=[]
    for m in JSON_LD.finditer(text or ""):
        try: p=json.loads(m.group(1).strip())
        except Exception: continue
        out.extend(p if isinstance(p,list) else [p])
    return out

def product_schema(text):
    q=list(jsonld_objects(text))
    while q:
        x=q.pop(0)
        if isinstance(x,list): q.extend(x); continue
        if not isinstance(x,dict): continue
        typ=x.get("@type")
        types={str(v).lower() for v in typ} if isinstance(typ,list) else {str(typ or "").lower()}
        if "product" in types: return x
        if isinstance(x.get("@graph"),list): q.extend(x["@graph"])
    return None

def offer(schema):
    o=schema.get("offers") if isinstance(schema,dict) else None
    if isinstance(o,list): return next((x for x in o if isinstance(x,dict)),None)
    return o if isinstance(o,dict) else None

def parse_price(schema,o):
    raw=(o.get("price") or o.get("lowPrice")) if isinstance(o,dict) else None
    cur=o.get("priceCurrency") if isinstance(o,dict) else None
    p=normalize_price(raw)
    if p is not None and p <= 0: p=None
    return p, clean(cur).upper() or "USD"

def parse_availability(schema,o):
    raw=""
    if isinstance(o,dict): raw=clean(o.get("availability") or o.get("itemAvailability")).lower()
    if not raw and isinstance(schema,dict): raw=clean(schema.get("availability")).lower()
    if "instock" in raw: return True,True,"IN_STOCK","JSON_LD_OFFER_AVAILABILITY"
    if "outofstock" in raw or "soldout" in raw: return False,True,"OUT_OF_STOCK","JSON_LD_OFFER_AVAILABILITY"
    if "preorder" in raw or "pre-order" in raw: return True,True,"PREORDER","JSON_LD_OFFER_AVAILABILITY"
    return False,False,"UNKNOWN","UNKNOWN"

def parse_title(schema,text):
    t=clean(schema.get("name")) if isinstance(schema,dict) else ""
    if t:return t
    m=OG_TITLE.search(text or "")
    if m:return clean(m.group(1))
    m=TITLE.search(text or "")
    return clean(m.group(1)) if m else ""

def parse_image(schema,text):
    img=schema.get("image") if isinstance(schema,dict) else None
    if isinstance(img,str) and img.strip(): return img.strip()
    if isinstance(img,list):
        for x in img:
            if isinstance(x,str) and x.strip(): return x.strip()
            if isinstance(x,dict) and clean(x.get("url")): return clean(x.get("url"))
    if isinstance(img,dict) and clean(img.get("url")): return clean(img.get("url"))
    m=OG_IMAGE.search(text or "")
    return clean(m.group(1)) if m else None

def priority(url):
    u=str(url or "").lower()
    return sum(10 for x in TCG_PRIORITY if x in u)

@retailer_adapter("bigcommerce")
class BigCommerceAdapter(RetailerAdapter):
    platform="bigcommerce"

    def __init__(self,*,domain,region="US",store_name=None,request_delay=DEFAULT_REQUEST_DELAY,max_product_pages=MAX_PRODUCT_PAGES):
    ...
    async def _get(self,session,url):
        ...
        try:
            async with session.get(url,timeout=aiohttp.ClientTimeout(total=DEFAULT_TIMEOUT),allow_redirects=True) as r:
                if r.status>=400:
            ...
            self.diagnostics["pages_failed"]+=1
            self.diagnostics["last_error"]=f"{type(e).__name__}: {e}"
            return None

    async def _discover(self,session):
        queue=[urljoin(self.base_url+"/",p.lstrip("/")) for p in SITEMAP_PATHS]
        visited=set(); urls=set()
        while queue and len(visited)<MAX_SITEMAPS and len(urls)<MAX_DISCOVERED_URLS:
            u=queue.pop(0)
            if u in visited: continue
            visited.add(u)
            text=await self._get(session,u)
            if not text: continue
            locs=[clean(x) for x in LOC.findall(text)]
            if not locs: continue
            self.diagnostics["sitemaps_seen"]+=1
            for loc in locs:
                if not loc or not same_domain(loc,self.domain): continue
                low=loc.lower()
                if low.endswith(".xml") or "sitemap" in low:
                    if loc not in visited and loc not in queue: queue.append(loc)
                else:
                    urls.add(loc)
                    if len(urls)>=MAX_DISCOVERED_URLS: break
        ...
        print(f"BIGCOMMERCE DISCOVERY | Store={self.store_name} | Sitemaps={self.diagnostics['sitemaps_seen']} | TotalURLs={len(urls)} | SelectedForFetch={len(selected)}")
        return selected
    ...
    async def fetch_products(self):
        ...
        raw=[]
        async with aiohttp.ClientSession(headers=headers,connector=aiohttp.TCPConnector(limit=4,limit_per_host=2)) as session:
            for url in await self._discover(session):
                text=await self._get(session,url)
                if not text: continue
                schema=product_schema(text)
                if not isinstance(schema,dict): continue
                raw.append({"url":url,"html":text,"schema":schema})
                self.diagnostics["product_pages_successful"]+=1
                await asyncio.sleep(self.request_delay)
        print(f"BIGCOMMERCE FETCH COMPLETE | Store={self.store_name} | ProductURLs={self.diagnostics['product_urls_discovered']} | ProductPages={self.diagnostics['product_pages_successful']} | RawProducts={len(raw)}")
        return raw

    def normalize_product(self,p):
        if not isinstance(p,dict):
            self.diagnostics["products_rejected"]+=1; return None
        url=clean(p.get("url")); text=p.get("html") or ""; schema=p.get("schema")
        if not url or not isinstance(schema,dict):
            self.diagnostics["products_rejected"]+=1; return None
        title=parse_title(schema,text); game=classify_game(title)
        if not game:
            self.diagnostics["products_rejected"]+=1; return None
        ...
        ext=clean(schema.get("productID") or schema.get("mpn") or sku) or None
